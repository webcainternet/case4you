 ...
 <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form_carrinho">
  ...
  <div class="cep">
  	<div class="campos-frete">
            ...
            <input name="insira" class="insira" type="text"  value="<?php if(isset($this->session->data['cep_padrao'])){echo $this->session->data['cep_padrao'];} ?>" maxlength="5" />
            <input name="digito" class="digito" type="text" value="<?php if(isset($this->session->data['cep_padrao_digito'])){echo $this->session->data['cep_padrao_digito'];} ?>" maxlength="3" />
            <input name="ok-entrega" class="ok-entrega" type="button" value="OK" />
            <a class="text naoSeiCep">> Não sei meu CEP</a>
        </div> 
	<div id="formas_envio" style="display:none;"></div>
	<script type="text/javascript">

	$(document).ready(function(){

	$('.tipo-entrega').html("<div style=' color: #878787; float: left;  font-family: museo-700 !important; font-size: 13px; font-weight: bold; position: relative;'>AGUARDE... <br /><br />"+
				"Verificando meios de entrega</div>");
	$('#payment-method').hide();
	$('.ok-entrega').hide();
	...
	<div class="tipo-entrega">
        <!--    <div class="text-entrega">TIPO DE ENTREGA</div>
            <input name="type_shipping" type="radio" value="free" checked="checked" class="radio" />
  	...
  	<div class="titu">1 - LOGIN</div>
        <?php if (!$logged) { ?>
            <div id="login" class="right login">
                ...
                <br />
                <a href="index.php?route=account/register" class="voltar-carrinho">CADASTRE-SE</a>
            </div>
  ...
  <div class="endereco"<?php if (!$logged) { ?> style="display:none;"<?php } ?>>
  	<div class="titu">2 - ENDEREÇO</div>
		...
		<input type="button" id="seta_cobranca" class="botao_endereco" />
			...
			<input type="button" id="seta_entrega" class="botao_endereco" />

            <ul class="hide listagem_endereco" id="list_end_entrega"><li>Carregando</li></ul>
            <div id="entrega" style="cursor:pointer;">
         ...
         <div class="novoEndereco hide">
            ...
            <span class="required">*</span> <label for="formEndDescription">Tipo de Endereço:</label><br />
            <!--  <input type="text" class="large-field" value="" name="description" id="formEndDescription" />  -->
            <?php  $tipos_endereco = array("Casa", "Escritório", "Apartamento", "Outros"); ?>
            <select name="description">
                <?php foreach($tipos_endereco as $endereco) { ?>
                <option value="<?php echo $endereco; ?>"><?php echo $endereco; ?></option>
                <?php } ?>
            </select>
            <br />
...
<div style="width:300px; float:left;">	
  ...
  <div class="buttons" id="payment-method">
	...
	<a class="button_cart" id="button-payment-method">
		<?php echo $button_checkout; ?>
	</a>
	
	<?php }else{ ?>
	
	<div class="pagamento-seguro" style="overflow:hidden;">
		<div class="titu" style="text-align:left;">
		CADASTRE-SE OU FAÇA O LOGIN<br />
...
$('#entrega').live('click',function(){
	var cep = $(this).find('.postcode').html();	
	cep = cep.replace("CEP: ", "");	
	cep = cep.replace("-", "");	
    ...
    var tipo = $('input[name=type_shipping]:checked').val();		    
			
    //alert(tipo);
	
    
    calcularFrete(cep1+cep2, tipo, function() {});
...
function efetuarLogin() {
    $.ajax({
        ...
        success: function(json) {
            ...
            htmlEntrega += '<input type="hidden" name="hd_cep"e class="hd_cep" value="' + ende.postcode + '" />';
...
$("#list_end_entrega li a").live("click", function() {
    ...
    var ende = $(enderecos[ind])[0];
	 ...
	 //console.debug(ende.postcode.split('-'));
    

	
    var cep2 = ende.postcode.replace('-','');
   //alert(cep2);		
 

   // inseri no campo de frente os dados para nao ficar nulo.	
   $('.insira').val(cep2.substr(0,5)); 
   $('.digito').val(cep2.substr(5,8));		

   //console.debug(cep2.substr(0,5));		
   //console.debug(cep2.substr(5,8));
    ...
    calcularFrete(ende.postcode, tipo, function() {});
});
...
$('#button-payment-method').live('click', function() {
    ...
    if(($('.insira').val() == "")  || ($('.insira').val().length > 5) || ($('.digito').val() =="") || ($('.digito').val().length < 3 )){
	
		alert("Preencha o Cep para o frete ou escolha um endereço para entrega");
    ...
    $.ajax({
        ...
        success: function() {
            $.ajax({
                ...
                dataType: 'json',
                beforeSend: function() {
                    $('#button-payment-method').after('<iframe class="frameconv" src="http://wewoodbrasil.com.br/conversion.html"></iframe>');
...
$('.insira').live('focusout',function(){
	if(($('.insira').val() != "")  && ($('.insira').val().length == 5)){	
		$('.insira').css('border','none');
	}
});


$('.digito').live('focusout',function(){
	if(($('.digito').val() !="") && ($('.digito').val().length == 3 )){
		$('.digito').css('border','none');
	}
});
...
$('.ok-entrega, .tipo-entrega .radio').live('click', function() {
    ...
    if(($('.insira').val() == "")  || ($('.insira').val().length > 5) || ($('.digito').val() =="") || ($('.digito').val().length < 3 )){
		$('.insira').css('border','1px solid red');		
		$('.digito').css('border','1px solid red');		
...
function calcularFrete(cep, tipo, callback) {
    $.ajax({
		...
		//console.debug(fretes);
                ...
                }else if(fretes["code"] != undefined){
		    var item = fretes["code"];
                }
...
$(document).ready(function(){
  // recupera o CEP e adiciona nos campos de cep e digito
  var cep = $(".postcode").html();
  cep = cep.split('CEP: ');
  cep = cep[1].split('-');
  $('.campos-frete .insira').attr('value', cep[0]);
  $('.campos-frete .digito').attr('value', cep[1]);
});
