	...
	public function validate() {
		...
		if (!$json) {
			...
			$customer_info = $this->model_account_customer->getCustomerByEmail($this->request->post['email']);
                        ...
                        $json["addresses"] = $this->customer->getAddresses();
                        $json["address"] = $this->customer->getAddress();                        

                        $this->session->data["shipping_address_id"] = $json["address"]["address_id"];
                        $this->session->data["payment_address_id"] = $json["address"]["address_id"];                                        
			...
			$address_info['postcode'] = str_replace("-","",$address_info['postcode']);
			$this->session->data["cep_padrao"] = substr($address_info['postcode'],0,5);
			$this->session->data["cep_padrao_digito"] = substr($address_info['postcode'],5,3);
		...
		if (!$json) {
			...
			if ($address_info) {
				if ($this->config->get('config_tax_customer') == 'shipping') {
					...
					$this->session->data['ivo'] = $address_info['postcode'];	
