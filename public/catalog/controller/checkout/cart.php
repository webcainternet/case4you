...
class ControllerCheckoutCart extends Controller {
    ...
    public function getCidades() {
        ...
        $id_estado = (!empty($this->request->post["estado"])) ? $this->request->post["estado"] : $this->request->get["estado"];
        $cities = $this->model_localisation_city->getCitiesByZoneId($id_estado);
        ...
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                ...
                if ($this->customer->isLogged()) {
                    $this->data["addresses"] = $this->customer->getAddresses();
                    $this->data["address"] = $this->customer->getAddress();
                    $this->session->data["shipping_address_id"] = $this->data["address"]["address_id"];
                    $this->session->data["payment_address_id"] = $this->data["address"]["address_id"];
                    $this->session->data["shipping_method"] = "sedex";
                    $this->session->data["payment_method"] = "pagseguro";
                }
            ...
            $this->data["customer_id"] = ($this->customer->getId() > 0) ? $this->customer->getId() : 0;
    ...
    public function getCep() {
        $logradouro = (!empty($this->request->post["logradouro"])) ? $this->request->post["logradouro"] : $this->request->get["logradouro"];
        $cidade = (!empty($this->request->post["cidade"])) ? $this->request->post["cidade"] : $this->request->get["cidade"];
        $uf = (!empty($this->request->post["estado"])) ? $this->request->post["estado"] : $this->request->get["estado"];
        $uf = strtolower($uf);

        $string = "SELECT * FROM " . $uf . " WHERE SOUNDEX(cidade) = SOUNDEX('" . $cidade . "')  AND (CONCAT(tp_logradouro, ' ', logradouro) LIKE '%" . $logradouro . "%' OR SOUNDEX(CONCAT(tp_logradouro, ' ', logradouro)) = SOUNDEX('" . $logradouro . "') OR SOUNDEX(logradouro) = SOUNDEX('" . $logradouro . "'));";

        $conn = mysql_connect("homebrasil.com", "homebr_wscep", "hb*euroset%cep");
        mysql_select_db("homebr_cep", $conn);
        $query = mysql_query($string);
        $enderecos = array();
        while ($endereco = mysql_fetch_assoc($query)) {
            $endereco["logradouro"] = htmlentities($endereco["logradouro"]);
            $endereco["cidade"] = htmlentities($endereco["cidade"]);
            $endereco["bairro"] = htmlentities($endereco["bairro"]);
            $enderecos[] = $endereco;
        }
    ...
    private function validateCoupon() {
        ...
        //echo "entrou no cupom";
        //die;
        $this->load->model('checkout/coupon');

        $coupon_info = $this->model_checkout_coupon->getCoupon($this->request->post['coupon']);
        //echo $this->request->post['coupon'];
        //echo "<pre>";
        //print_r($coupon_info);
        //die;
    ...
    public function add() {
        ...
        if ($product_info) {
            ...
            if (!$json) {
                if($this->cart->add($this->request->post['product_id'], $quantity, $option)){
			echo "adicionou";
		}
    ...
    public function quote_by_cep() {
        ...
        $this->request->post["country_id"] = 30;
        ...
        if (!$json) {
            ...
            foreach ($results as $result) {
                ...
                if ($this->request->post["type"] == "frete grátis") {
                    $frete_gratis = true;
                    $result['code'] = "free";
                    break;
//                }else if($coupon_info['shipping'] == 1){
//                    $frete_gratis = true;
//                    $result['code'] = "free";
//                    break;
                } else {
                    $frete_gratis = false;
                    $result['code'] = "correios";
                    break;
                }
            }
            //echo $this->request->post["type"];
            //echo $result['code'];
            //	die;
            ...
            if ($frete_gratis) {
                ...
                if ($this->config->get($result['code'] . '_status')) {
                    ...
                    $quote = $this->{'model_shipping_' . $result['code']}->getQuoteByCep($address_data, $this->request->post["type"]);
